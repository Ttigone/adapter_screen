...
/**
 * @brief AdaptiveMixin<BaseWidget> — CRTP mix-in template for screen adaptation.
 *
 ...
 *   - Responds to QWindow::screenChanged (window dragged to a different monitor).
...
public:
    explicit AdaptiveMixin(QWidget* parent = nullptr)
        : BaseWidget(parent)
    {
        ...
        QObject::connect(
            ScreenAdapter::Instance(), &ScreenAdapter::ScaleChanged,
            this, [this] { OnAdaptLayout(); });
    }

    // ─────────────────────────────────────────────────────────────────────
    //  Geometry initialisation strategies
    // ─────────────────────────────────────────────────────────────────────
    ...
    /**
     * @brief Strategy B: size the widget as a fraction of the screen area and centre it.
...
protected:
    ...
    virtual void OnAdaptLayout()
    {
        if (is_full_screen_) {
            ...
            ScreenAdapter::Instance()->ResizeByScreenPercent(
                this, screen_w_pct_, screen_h_pct_);
            ScreenAdapter::Instance()->CenterOnScreen(this);
        } else if (design_rect_.isValid()) {
            ScreenAdapter::Instance()->SetScaledGeometry(
                this,
                design_rect_.x(),     design_rect_.y(),
                design_rect_.width(), design_rect_.height());
        }
...
private:
    ...
    void ConnectScreenSignal()
    {
        ...
        if (screen_conn_)
            QObject::disconnect(screen_conn_);
...
// =============================================================================
//  Android full-screen convenience base class
// =============================================================================
#ifdef SA_ANDROID
/**
 * @brief AndroidFullWidget — convenience base for always-immersive Android views.
 *
