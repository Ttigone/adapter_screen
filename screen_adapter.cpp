...
// ─────────────────────────────────────────────────────────────────────────────
//  Android JNI headers (platform + version guards)
// ─────────────────────────────────────────────────────────────────────────────
#ifdef SA_ANDROID
#  ifdef SA_QT6
...
#    endif
#  else
     // Qt 5: QtAndroid + QAndroidJniObject
...
//  Constructor
// =============================================================================
ScreenAdapter::ScreenAdapter(QObject* parent)
    : QObject(parent)
{
    connect(qApp, &QGuiApplication::primaryScreenChanged,
            this, &ScreenAdapter::OnPrimaryScreenChanged);
}

// =============================================================================
//  High-DPI policy  (call BEFORE QApplication)
// =============================================================================
void ScreenAdapter::SetupHighDpiPolicy()
{
#ifdef SA_QT5
    // Qt5: manually enable logical-pixel scaling
...
void ScreenAdapter::Init(const Config& cfg)
{
    ...
    // Track primary screen geometry changes (system DPI / resolution change)
    if (primary) {
        connect(primary, &QScreen::geometryChanged,
                this, &ScreenAdapter::OnScreenGeometryChanged,
                Qt::UniqueConnection);
    }

    qDebug().nospace()
        << "[ScreenAdapter] Init OK"
        << " | Design=" << cfg_.design_width << "x" << cfg_.design_height
        << " | Screen=" << screen_size_.width() << "x" << screen_size_.height()
        << " | DPR="    << dpr_
        << " | DPI="    << screen_dpi_
        << " | scale="  << s_ << " (sx=" << sx_ << " sy=" << sy_ << ")"
...
// =============================================================================
//  Detect screen parameters and compute scale factors
// =============================================================================
void ScreenAdapter::Detect(QScreen* screen)
{
    ...
    // Both Qt5 (with AA_EnableHighDpiScaling) and Qt6 report geometry in
    // logical pixels — that is exactly the coordinate space we want.
    ...
    const qreal sy = static_cast<qreal>(screen_size_.height()) / cfg_.design_height;
    sx_ = sx;
    sy_ = sy;
    // Uniform scale: min-axis keeps all content visible without clipping.
    // Stretch scale: each axis independent, useful for always-full-screen UIs.
...
// =============================================================================
//  Detect device type (phone / tablet / desktop) from physical diagonal
// =============================================================================
void ScreenAdapter::DetectDeviceType(QScreen* screen)
{
    if (!screen) return;

    const QSizeF phys      = screen->physicalSize();  // millimetres
    const qreal  diag_mm   = qSqrt(phys.width()  * phys.width()
...
{
    if (!screen) return s_;
    const qreal sx = static_cast<qreal>(screen->geometry().width())  / cfg_.design_width;
    const qreal sy = static_cast<qreal>(screen->geometry().height()) / cfg_.design_height;
...
{
    if (!w) return QApplication::primaryScreen();
...
//  Widget helpers
// =============================================================================
void ScreenAdapter::SetScaledGeometry(QWidget* w,
                                      int x, int y, int width, int height) const
{
    if (w) w->setGeometry(Svx(x), Svy(y), Sv(width), Sv(height));
}

void ScreenAdapter::CenterOnScreen(QWidget* w, QScreen* screen) const
{
    if (!w) return;
    if (!screen) screen = ScreenOf(w);
...
void ScreenAdapter::ResizeByScreenPercent(QWidget* w,
                                          qreal w_pct, qreal h_pct,
                                          QScreen* screen) const
{
    ...
    w->resize(qRound(avail.width()  * w_pct),
              qRound(avail.height() * h_pct));
}

void ScreenAdapter::ApplyFontScaleRecursive(QWidget* w) const
{
    if (!w || qFuzzyCompare(font_scale_, 1.0)) return;

    static const char* k_orig_pt_key = "_sa_orig_pt";

    const auto apply_one = [this, k_orig_pt_key](QWidget* widget) {
        QFont f = widget->font();
        if (f.pointSize() <= 0) return;  // skip pixel-size fonts

        const QVariant stored  = widget->property(k_orig_pt_key);
        const int      orig_pt = stored.isValid() ? stored.toInt() : f.pointSize();
        if (!stored.isValid())
            widget->setProperty(k_orig_pt_key, orig_pt);  // save original once
    ...
    apply_one(w);
    // Direct children only; each level calls this recursively via AdaptiveMixin
...
{
    // Primary screen resolution or system DPI changed (e.g. user changed
    // Windows display scaling in Settings).
    Detect(nullptr);
    emit ScaleChanged();
}
...
static void ApplyWindowFlagJNI(const char* method, jint flag)
{
    ...
    QJniObject window = activity.callObjectMethod(
        "getWindow", "()Landroid/view/Window;");
    if (window.isValid())
        window.callMethod<void>(method, "(I)V", flag);
}
...
static void ApplyImmersiveJNI(jint window_flags, jint ui_flags)
{
    QtAndroid::runOnAndroidThread([window_flags, ui_flags]() {
        ...
        QAndroidJniObject decor_view = window.callObjectMethod(
            "getDecorView", "()Landroid/view/View;");
        if (decor_view.isValid())
            decor_view.callMethod<void>("setSystemUiVisibility", "(I)V", ui_flags);
    });
